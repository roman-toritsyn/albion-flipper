...
export function CraftRow({ flip, taxRate, index = 0, now = Date.now() }: Props) {
  ...
  const profit = craftProfit(flip.bmBuy, flip.cost, taxRate);
  const roi = craftRoi(flip.bmBuy, flip.cost, taxRate);
  const net = craftNet(flip.bmBuy, taxRate);
    ...
    >
      <div className="grid gap-4 sm:grid-cols-[auto_1fr] sm:items-center lg:grid-cols-[minmax(200px,1.2fr)_repeat(3,minmax(120px,1fr))] lg:gap-6">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <div className="mt-1.5 flex flex-wrap items-center gap-1.5">
              ...
              <span className="inline-block rounded-md border border-border px-1.5 py-0.5 text-[10px] uppercase tracking-wider text-muted">
                {flip.family === "cape" ? t("craftFamilyCapes") : t("craftFamilyRoyal")}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-3 lg:contents">
          <div>
            <p className="text-[10px] uppercase tracking-[0.16em] text-muted">{t("craftCost")}</p>
            <p className="mt-1 font-[family-name:var(--font-mono)] text-lg tabular text-text">
              {formatSilver(flip.cost)}
            </p>
            <p className={`text-[11px] ${staleIngredients ? "text-stale" : "text-muted"}`}>
          ...
          <div>
            ...
            <p className="text-[11px] text-muted">{formatAgeLabelT(flip.bmBuyDate, now, t)}</p>
          </div>
